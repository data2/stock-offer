        ...
        for i, stock in enumerate(hot_stocks, 1):
            ...
            try:
                ...
                table.add_row([
                    ...
                    f"{Style.BRIGHT if 'peak_day' in analysis and analysis['peak_day'] else ''}{Fore.RED if 'peak_day' in analysis and analysis['peak_day'] else Fore.GREEN}{analysis.get('peak_day', '无')}{Style.RESET_ALL}",
                    f"{Style.BRIGHT if (analysis.get('adjust_days', 0) >= self.MIN_CONSECUTIVE_DAYS) else ''}{Fore.RED if (analysis.get('adjust_days', 0) >= self.MIN_CONSECUTIVE_DAYS) else Fore.GREEN}{analysis.get('adjust_days', 0)}{Style.RESET_ALL}",
                    f"{Style.BRIGHT if (analysis.get('current_volume_ratio', 0) < self.volume_threshold and analysis.get('current_volume_ratio', 0) > 0) else ''}{Fore.RED if (analysis.get('current_volume_ratio', 0) < self.volume_threshold and analysis.get('current_volume_ratio', 0) > 0) else Fore.YELLOW if analysis.get('current_volume_ratio', 0) < 0.5 else Fore.GREEN}{analysis.get('current_volume_ratio', 0):.2f}{Style.RESET_ALL}",
                    f"{Style.BRIGHT if analysis.get('is_near_ma', False) else ''}{Fore.RED if analysis.get('is_near_ma', False) else Fore.GREEN}{analysis.get('near_ma', '无')}{Style.RESET_ALL}",
                    f"{Style.BRIGHT if (analysis.get('ma_score', 0) > self.MIN_MA_SCORE) else  ''}{Fore.RED if (analysis.get('ma_score', 0) > self.MIN_MA_SCORE) else Fore.GREEN}{analysis.get('ma_score', 0):.2f}{Style.RESET_ALL}",
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == '1':
            ...
            top_n = input("请输入要分析的热股数量(默认500): ").strip()
            try:
                top_n = int(top_n) if top_n else 500
